...
class ConnectionManager {
    constructor({ log, getPort, getCustomTexts }) {
        ...
        // Swarm Mode: sidebar target tracking
        this._sidebarTargetId = null;
        this._sidebarWsUrl = null;
        this._swarmScript = null; // Held in memory only — never written to disk
    ...
    async connect() {
        ...
        try {
            ...
            // ── RECON TASK 1: One-time target dump (remove after sidebar ID is known) ──
            if (!ConnectionManager._reconDone) {
                ConnectionManager._reconDone = true;
                this.log('=== [RECON] ALL CDP TARGETS ===');
                targets.forEach(t => {
                    this.log(`[RECON] id=${t.id} type=${t.type} title="${t.title || ''}" url=${(t.url || '').substring(0, 120)}`);
                });
                this.log('=== [RECON] END — look for sidebar/agent-manager target above ===');
            }
            // ── END RECON ──────────────────────────────────────────────────────────────
    ...
    async _handleNewTarget(targetInfo) {
        ...
        try {
            ...
            // ── Swarm Mode: Agent Manager detection ───────────────────────────
            // The Agent Manager standalone window loads workbench-jetski-agent.html.
            // We inject a secondary swarm observer that watches the conversation
            // sidebar for pending spinners and auto-clicks to unblock background chats.
            if ((url || '').includes('jetski-agent')) {
                this._sidebarTargetId = targetId;
                this._sidebarWsUrl = webSocketDebuggerUrl;
                this.log(`[Swarm] Agent Manager identified (${shortId}) — injecting swarm observer in 1.5s`);
                setTimeout(() => this._injectManagerSwarmScript(webSocketDebuggerUrl, shortId), 1500);
            }
            // ─────────────────────────────────────────────────────────────────
        } catch (e) {
    ...
    // ─── Swarm Mode: Agent Manager Swarm Observer ─────────────────────

    /**
     * Injects the Swarm Observer into the Agent Manager window.
     * Watches for pending-chat indicators (spinner + bell icons)
     * and clicks through them one-by-one, re-scanning the DOM fresh
     * after each navigation to avoid stale element references.
     */
    _injectManagerSwarmScript(wsUrl, shortId) {
        // Swarm script is served by aa-swarm Cloudflare Worker (Pro license required).
        // No IIFE is bundled here. Use injectSwarmObserver(script) to activate.
        this.log(`[Swarm] Pro license required for Swarm Mode. (${shortId || 'n/a'})`);
    }

    /**
     * Public API: inject an externally-provided swarm script (e.g. from aa-swarm Cloudflare Worker).
     * Falls back to the built-in _injectManagerSwarmScript if no script is provided.
     */
    async injectSwarmObserver(script, targetId) {
        const tid = targetId || this._sidebarTargetId;
        const wsUrl = tid
            ? (this.sessions.get(tid)?.wsUrl || this._sidebarWsUrl)
            : this._sidebarWsUrl;

        if (!wsUrl) {
            this.log('[Swarm] No Agent Manager target identified yet');
            return false;
        }

        if (!script) {
            this._injectManagerSwarmScript(wsUrl, tid?.substring(0, 6));
            return false; // no script = Pro required
        }

        this._swarmScript = script;
        try {
            // Inject configurable idle time — users can set autoAcceptV2.swarmIdleSeconds
            const vscode = require('vscode');
            const idleMs = vscode.workspace.getConfiguration('autoAcceptV2').get('swarmIdleSeconds', 15) * 1000;
            const dynamicScript = `window.__AA_SWARM_IDLE_MS = ${idleMs};\n` + script;

            const result = await this._workerEval(wsUrl, dynamicScript);
            const val = result?.result?.result?.value || result?.result?.value || 'unknown';
            this.log(`[Swarm] External script injected → ${val}`);

            if (val === 'swarm-observer-installed' || val === 'already-active') {
                // ── Interval leak fix: clear any existing poller before creating a new one ──
                if (this._swarmLogInterval) clearInterval(this._swarmLogInterval);
                this._swarmLogInterval = setInterval(() => {
                    this._workerEval(wsUrl,
                        'var q=window.__AA_SWARM_LOG;if(q&&q.length){window.__AA_SWARM_LOG=[];JSON.stringify(q);}else{"[]"}',
                        2000)
                        .then(res => {
                            const raw = res?.result?.result?.value || res?.result?.value || '[]';
                            let lines;
                            try { lines = JSON.parse(raw); } catch(e) { lines = []; }
                            lines.forEach(l => this.log(`[SwarmMode${l}`));
                        }).catch(() => {});
                }, 3000);
                return true;
            }
            ...
            this.log(`[Swarm] External injection failed: ${e.message}`);
            return false;
        }
    }
